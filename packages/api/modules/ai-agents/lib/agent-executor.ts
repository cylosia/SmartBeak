...
import { createAnthropic, generateText, openai, streamText } from "@repo/ai";
import type {
	AiAgentConfig,
	AiMemoryContext,
	WorkflowGraph,
...
function topologicalSort(graph: WorkflowGraph): WorkflowNode[] {
	...
	if (sorted.length !== nodes.length) {
		throw new ORPCError("BAD_REQUEST", {
			message:
				"Workflow graph contains a cycle. Please remove circular dependencies between agents.",
		});
	}
...
export async function* executeWorkflow(
	...
	try {
		...
		const agentNodes = sortedNodes.filter(
			(n) => n.type === "agent" && n.agentId,
		);

		if (agentNodes.length === 0) {
			throw new ORPCError("BAD_REQUEST", {
				message: "Workflow has no agent nodes to execute.",
			});
		}
		...
		for (const node of agentNodes) {
			...
			compressAndUpdateMemory(
				...
				logger.warn(
					`[agent-executor] Failed to compress memory for agent ${agent.id}:`,
					err,
				);
			});
		}
...
async function compressAndUpdateMemory(
	...
	try {
		...
		logger.warn(
			`[agent-executor] Memory compression failed for agent ${agentId}:`,
			err,
		);
	}
}
