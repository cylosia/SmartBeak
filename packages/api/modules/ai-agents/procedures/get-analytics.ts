...
import { GetAnalyticsInputSchema, getAgentAnalytics } from "@repo/database";
...
export const getAgentAnalyticsProcedure = protectedProcedure
	.route({
		...
		tags: ["AI Agents"],
		summary:
			"Get agent usage, performance, and cost analytics for an organization",
	})
	...
	.handler(async ({ context: { user }, input }) => {
		...
		const analytics = await getAgentAnalytics(org.id, {
			startDate,
			endDate,
		});

		// Format cost from cents to dollars for display
		const totalCostUsd = Number(analytics.totals.totalCostCents ?? 0) / 100;
		const successRate =
		...
		return {
			summary: {
				...
				totalInputTokens: Number(
					analytics.totals.totalInputTokens ?? 0,
				),
				totalOutputTokens: Number(
					analytics.totals.totalOutputTokens ?? 0,
				),
