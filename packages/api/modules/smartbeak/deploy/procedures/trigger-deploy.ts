	...
	.handler(async ({ context: { user }, input }) => {
		...
		if (!token) {
			throw new ORPCError("PRECONDITION_FAILED", {
				message:
					"VERCEL_TOKEN is not configured. Deployment is unavailable.",
			});
		}
		...
		logger.info(
			`[trigger-deploy] deploy v${nextVersion} started for domain=${domain.id}`,
		);
		...
		(async () => {
			try {
				...
				if (!deployRes.ok) {
					...
					logger.error(
						`[trigger-deploy] Vercel API ${deployRes.status}`,
						{ errText },
					);
					throw new ORPCError("BAD_GATEWAY", {
						message:
							"Deployment failed. Vercel API returned an error.",
					});
				}
					...
					if (!statusRes.ok) {
						if (statusRes.status >= 400 && statusRes.status < 500) {
							...
							if (consecutive4xx >= 3) {
								throw new ORPCError("BAD_GATEWAY", {
									message:
										"Deployment status check failed after multiple retries.",
								});
							}
						}
						continue;
