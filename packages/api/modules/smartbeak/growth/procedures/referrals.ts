...
export const getMyReferralsProcedure = protectedProcedure
	.route({
		method: "GET",
		path: "/smartbeak/growth/referrals/mine",
...
export const completeReferralProcedure = adminProcedure
	.route({
		method: "POST",
		path: "/smartbeak/growth/referrals/complete",
		tags: ["SmartBeak - Growth"],
		summary: "Complete a referral (admin)",
	})
	.input(
		z.object({
	...
	.handler(async ({ input }) => {
		...
		const completed = await completeReferral(
			referral.id,
			input.referredUserId,
		);
		if (!completed) {
			throw new ORPCError("NOT_FOUND", {
				message: "Referral could not be completed.",
			});
		}
		return completed;
	});
...
export const grantRewardProcedure = adminProcedure
	.route({
		method: "POST",
		path: "/smartbeak/growth/referrals/reward",
		tags: ["SmartBeak - Growth"],
		summary: "Grant a referral reward (admin)",
	})
	.input(GrantRewardInputSchema)
	.handler(async ({ input }) => {
		const rewarded = await grantReferralReward(
			input.referralId,
			input.rewardType,
			input.rewardValue,
		);
...
export const getReferralStatsByCodeProcedure = publicProcedure
	.route({
		method: "GET",
		path: "/smartbeak/growth/referrals/stats",
		tags: ["SmartBeak - Growth"],
