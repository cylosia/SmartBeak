...
export const linkedinAdapter: PublishAdapter = {
	...
	async publish(config, payload) {
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `LinkedIn API error ${res.status}: ${err}`,
			};
		}
...
export const tiktokAdapter: PublishAdapter = {
	...
	async publish(config, payload) {
			...
			"https://open.tiktokapis.com/v2/post/publish/text/check/",
			{
				...
				body: JSON.stringify({
					post_info: {
						title: payload.title,
						description:
							payload.excerpt ?? payload.body.slice(0, 2200),
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `TikTok API error ${res.status}: ${err}`,
			};
		}
...
export const instagramAdapter: PublishAdapter = {
	name: "instagram",
	async publish(config, payload) {
		const { accessToken, igUserId } = config as {
			accessToken: string;
			igUserId: string;
		};
		...
		if (!containerRes.ok) {
			...
			return {
				success: false,
				error: `Instagram container error ${containerRes.status}: ${err}`,
			};
		}
		const { id: creationId } = (await containerRes.json()) as {
			id: string;
		};
		// Step 2: publish
		...
		if (!publishRes.ok) {
			...
			return {
				success: false,
				error: `Instagram publish error ${publishRes.status}: ${err}`,
			};
		}
...
export const pinterestAdapter: PublishAdapter = {
	name: "pinterest",
	async publish(config, payload) {
		const { accessToken, boardId } = config as {
			accessToken: string;
			boardId: string;
		};
		const res = await fetchWithTimeout(
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `Pinterest API error ${res.status}: ${err}`,
			};
		}
		const data = (await res.json()) as { id?: string };
		return {
			...
			platformPostId: data.id,
			url: data.id
				? `https://www.pinterest.com/pin/${data.id}`
				: undefined,
		};
	},
};
...
export const vimeoAdapter: PublishAdapter = {
	...
	async publish(config, payload) {
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `Vimeo API error ${res.status}: ${err}`,
			};
		}
...
export const facebookAdapter: PublishAdapter = {
	name: "facebook",
	async publish(config, payload) {
		const { accessToken, pageId } = config as {
			accessToken: string;
			pageId: string;
		};
		const res = await fetchWithTimeout(
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `Facebook API error ${res.status}: ${err}`,
			};
		}
		const data = (await res.json()) as { id?: string };
		return { success: true, platformPostId: data.id };
...
export const wordpressAdapter: PublishAdapter = {
	...
	async publish(config, payload) {
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `WordPress API error ${res.status}: ${err}`,
			};
		}
...
export const soundcloudAdapter: PublishAdapter = {
	...
	async publish(config, payload) {
		...
		if (!res.ok) {
			...
			return {
				success: false,
				error: `SoundCloud API error ${res.status}: ${err}`,
			};
		}
