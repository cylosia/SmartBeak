...
export const getPublishAnalyticsProcedure = protectedProcedure
  ...
  .handler(async ({ context: { user }, input }) => {
    ...
    const byPlatform: Record<
      string,
      { views: number; engagement: number; clicks: number; impressions: number; posts: number }
    > = {};
    for (const r of rows) {
      const t = r.target as string;
      if (!byPlatform[t]) byPlatform[t] = { views: 0, engagement: 0, clicks: 0, impressions: 0, posts: 0 };
