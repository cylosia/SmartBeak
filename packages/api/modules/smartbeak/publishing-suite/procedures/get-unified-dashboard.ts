...
export const getUnifiedDashboardProcedure = protectedProcedure
  ...
  .handler(async ({ context: { user }, input }) => {
    ...
    for (const row of statusSummary) {
      ...
      if (!byPlatform[t]) byPlatform[t] = { total: 0, published: 0, failed: 0 };
      byPlatform[t].total += row.n;
      if (row.status === "published") byPlatform[t].published += row.n;
      if (row.status === "failed") byPlatform[t].failed += row.n;
    }
