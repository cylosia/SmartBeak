...
async function logMaintenanceOperation(
  ...
  try {
    ...
    logger.error('Failed to log maintenance operation', undefined, { error: err instanceof Error ? err.message : String(err) });
  }
}
  ...
  if (!dryRun) {
    for (const table of tablesToVacuum) {
      ...
      try {
        ...
        logger.error('Skipping table with invalid name from db_vacuum_statistics', undefined, { table });
        continue;
      }
