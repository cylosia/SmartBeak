...
import type { AiAgentConfig, AiMemoryContext, WorkflowGraph } from "../zod-ai-agents";
...
export async function createAgent(data: {
  ...
  const rows = await db
    ...
    .values({
      ...
      config: data.config,
      memoryContext: { summary: "", facts: [], lastUpdatedAt: new Date().toISOString() },
...
export async function updateSession(
  ...
  const rows = await db
    ...
    .set({
      ...(data.status !== undefined && { status: data.status }),
      ...(data.outputData !== undefined && { outputData: data.outputData }),
