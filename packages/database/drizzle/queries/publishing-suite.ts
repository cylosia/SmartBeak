...
export async function getScheduledJobsInRange(domainId: string, from: Date, to: Date) {
  return db.query.publishingJobs.findMany({
    where: and(
...
export async function getFailedWebhookEvents(opts: { limit?: number; offset?: number } = {}) {
  return db.query.webhookEvents.findMany({
