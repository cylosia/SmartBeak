...
export const publishTarget = pgEnum("publish_target", [
	"web",
	"linkedin",
	"facebook",
	"instagram",
...
export const domains = pgTable(
	...
	(t) => [index("domains_org_id_idx").on(t.orgId)],
);
...
export const contentRevisions = pgTable(
	...
	(t) => [index("content_revisions_content_id_idx").on(t.contentId)],
);
...
export const mediaAssets = pgTable(
	...
	(t) => [index("media_assets_domain_id_idx").on(t.domainId)],
);
...
export const publishTargets = pgTable(
	...
	(t) => [index("publish_targets_domain_id_idx").on(t.domainId)],
);
...
export const publishAttempts = pgTable(
	...
	(t) => [index("publish_attempts_job_id_idx").on(t.jobId)],
);
...
export const seoDocuments = pgTable(
	...
	(t) => [index("seo_documents_domain_id_idx").on(t.domainId)],
);

export const keywordTracking = pgTable(
	"keyword_tracking",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		domainId: uuid("domain_id")
	...
	(t) => [
		...
		index("keyword_tracking_domain_updated_idx").on(
			t.domainId,
			t.lastUpdated,
		),
...
export const usageRecords = pgTable(
	...
	(t) => [index("usage_records_org_id_idx").on(t.orgId)],
);

export const monetizationDecaySignals = pgTable(
	"monetization_decay_signals",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		domainId: uuid("domain_id")
			.references(() => domains.id, { onDelete: "cascade" })
			.notNull(),
		decayFactor: numeric("decay_factor", {
			precision: 5,
			scale: 4,
		}).notNull(),
...
// 8. SmartDeploy
export const siteShards = pgTable(
	"site_shards",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		domainId: uuid("domain_id")
	...
	(t) => [index("site_shards_domain_id_idx").on(t.domainId)],
);

// 9. Diligence & Portfolio
export const diligenceChecks = pgTable(
	"diligence_checks",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		domainId: uuid("domain_id")
	...
	(t) => [index("diligence_checks_domain_id_idx").on(t.domainId)],
);

export const portfolioSummaries = pgTable(
	"portfolio_summaries",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		orgId: uuid("org_id")
	...
	(t) => [index("portfolio_summaries_org_id_idx").on(t.orgId)],
);
...
export const webhookEvents = pgTable(
	...
	(t) => [
		index("webhook_events_processed_outbox_idx").on(
			t.processed,
			t.outboxStatus,
		),
		index("webhook_events_outbox_created_idx").on(
			t.outboxStatus,
			t.createdAt,
		),
	],
...
export const buyerSessions = pgTable(
	...
	(t) => [index("buyer_sessions_domain_id_idx").on(t.domainId)],
);
...
export const timelineEvents = pgTable(
	...
	(t) => [index("timeline_events_domain_id_idx").on(t.domainId)],
);
...
export const guardrails = pgTable(
	...
	(t) => [index("guardrails_org_id_idx").on(t.orgId)],
);
...
export const featureFlags = pgTable(
	...
	(t) => [index("feature_flags_org_id_idx").on(t.orgId)],
);
...
export const onboardingProgress = pgTable(
	...
	(t) => [index("onboarding_progress_org_id_idx").on(t.orgId)],
);
