...
export const vimeoConfigSchema = z.object({
	accessToken: z.string().min(1),
	privacy: z
		.enum(["anybody", "nobody", "password", "unlisted"])
		.default("anybody"),
});
...
export const platformConfigSchema = z.discriminatedUnion("target", [
	...
	z.object({
		target: z.literal("facebook"),
		config: z.object({ accessToken: z.string(), pageId: z.string() }),
	}),
	z.object({
		target: z.literal("wordpress"),
		config: z.object({
	...
	z.object({
		target: z.literal("soundcloud"),
		config: z.object({ accessToken: z.string() }),
	}),
]);
