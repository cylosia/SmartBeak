...
import { getDifficultyTier, getDecayHealth } from "../lib/seo-utils";
...
function generateScoreTrend(currentScore: number, days = 30) {
  return Array.from({ length: days }, (_, i) => {
    ...
    const score = Math.max(0, Math.min(100, Math.round(currentScore * 0.6 + growth + noise)));
    const date = new Date();
...
export function SeoDashboard({
  ...
  const stats = useMemo(() => {
    ...
    const bestPosition = keywords.reduce(
      (best, k) => (k.position != null && (best === null || k.position < best) ? k.position : best),
      null as number | null,
    );
    const decayWarnings = keywords.filter((k) => getDecayHealth(k.decayFactor).level >= 2).length;
  ...
  const clusters = useMemo(() => {
    ...
    const medium = keywords.filter((k) => (k.difficulty ?? 0) >= 30 && (k.difficulty ?? 0) < 70).length;
  ...
  const alerts = useMemo(() => {
    const items: { type: "warning" | "danger" | "info"; message: string; icon: typeof AlertTriangleIcon }[] = [];
    ...
    const declining = keywords.filter((k) => getDecayHealth(k.decayFactor).level >= 3);
    ...
    if ((seoDoc?.score ?? 0) < 40) {
      items.push({ type: "danger", message: "SEO score is critically low", icon: AlertCircleIcon });
    } else if ((seoDoc?.score ?? 0) < 70) {
      items.push({ type: "warning", message: "SEO score needs improvement", icon: AlertTriangleIcon });
    }
    if (keywords.length === 0) {
      items.push({ type: "info", message: "No keywords tracked yet — add keywords to get started", icon: SearchIcon });
    }
    if (items.length === 0) {
      items.push({ type: "info", message: "All looking good! Keep monitoring your rankings.", icon: CheckCircle2Icon });
    }
    return items;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          subtitle={stats.decayWarnings > 0 ? "Keywords need attention" : "All healthy"}
          icon={AlertTriangleIcon}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-sm font-medium">Score Trend (30 days)</CardTitle>
          </CardHeader>
          <CardContent>
            <ChartContainer config={chartConfig} className="h-[220px] w-full">
              <AreaChart data={scoreTrend} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                <defs>
        ...
        <Card>
          ...
          <CardContent className="flex items-center justify-center">
            {clusters.length === 0 ? (
              <p className="text-sm text-muted-foreground py-8">No keyword data</p>
            ) : (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium">Alerts & Recommendations</CardTitle>
        </CardHeader>
        <CardContent>
