...
const columns: ColumnDef<KeywordRow>[] = [
  ...
  {
    accessorKey: "keyword",
    header: ({ column }) => <SortableHeader label="Keyword" column={column} />,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: ({ column }) => <SortableHeader label="Difficulty" column={column} />,
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${tier.bg} ${tier.color}`}>
            {tier.label}
          </span>
        </div>
      );
    },
    sortingFn: (a, b) => (a.original.difficulty ?? 0) - (b.original.difficulty ?? 0),
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (pos == null) return <span className="text-sm text-muted-foreground">—</span>;
      const TrendIcon = trend?.direction === "up" ? ArrowUpIcon : trend?.direction === "down" ? ArrowDownIcon : ArrowRightIcon;
      return (
        <div className="flex items-center gap-1.5">
          <TrendIcon className={`h-3.5 w-3.5 ${trend?.color ?? ""}`} />
          <span className="text-sm font-medium tabular-nums">#{pos}</span>
        </div>
      );
    },
  ...
  },
  {
    ...
    accessorFn: (row) => row.decayFactor ? Number.parseFloat(row.decayFactor) : 0,
    cell: ({ row }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        {table.getFilteredRowModel().rows.length} keyword{table.getFilteredRowModel().rows.length !== 1 ? "s" : ""} tracked
      </p>
    </div>
  );
}
