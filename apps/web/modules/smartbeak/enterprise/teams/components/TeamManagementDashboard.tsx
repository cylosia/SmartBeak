...
export function TeamManagementDashboard({
	...
	const [activeTab, setActiveTab] = useState<"members" | "activity">(
		"members",
	);
	...
	const selectedTeam =
		teamsQuery.data?.teams.find((t) => t.id === selectedTeamId) ??
		teamsQuery.data?.teams[0] ??
		null;
		...
		orpc.enterprise.teams.create.mutationOptions({
			onSuccess: () => {
				queryClient.invalidateQueries({
					queryKey: orpc.enterprise.teams.list.key({
						input: { organizationSlug },
					}),
				});
							...
							>
								...
								<DialogContent>
									<DialogHeader>
										...
										<DialogDescription>
											Create a new workspace team with
				...
				<Card className="lg:col-span-3">
					{!selectedTeam ? (
						<CardContent className="flex flex-col items-center justify-center py-16 text-center">
							...
							<p className="text-sm text-muted-foreground">
								Select a team to manage its members and
								activity.
							</p>
						</CardContent>
					) : (
						<>
							...
							<Tabs
								value={activeTab}
								onValueChange={(v) =>
									setActiveTab(v as "members" | "activity")
								}
							>
								<div className="px-6">
									<TabsList>
