...
export function SeoIntelligenceDashboard({
	...
	const { summary, keywords = [], clusters = [] } = dashboardQuery.data ?? {};

	const decayingCount = keywords.filter(
		(k) => Number.parseFloat(k.decayFactor ?? "1") < 0.5,
	).length;
							...
							<div className="space-y-4">
								<div className="flex items-center gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
									...
									<p className="text-sm text-amber-700 dark:text-amber-400">
										{decayingCount} keyword
										{decayingCount !== 1 ? "s are" : " is"}{" "}
										showing decay signals. Update content or
										refresh rankings to restore scores.
									</p>
								</div>
								<div className="rounded-xl border border-border overflow-hidden">
									<Table>
