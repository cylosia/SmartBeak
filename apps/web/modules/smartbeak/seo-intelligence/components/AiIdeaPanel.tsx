		...
		<Sheet open onOpenChange={(open: boolean) => !open && onClose()}>
			...
			>
				<SheetHeader className="mb-6">
					...
					<SheetDescription>
						Generate SEO-optimized content ideas with outlines,
						keyword targets, and SEO scores — powered by the Vercel
						AI SDK.
					</SheetDescription>
				</SheetHeader>
				...
				{ideas.length > 0 && (
					<div className="space-y-4">
						...
						{ideas.map((idea, i) => (
							<IdeaCard
								key={`idea-${idea.title.slice(0, 20)}-${i}`}
								idea={idea}
								index={i}
							/>
						))}
