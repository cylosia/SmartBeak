...
function SeverityIcon({ severity }: { severity: string }) {
	if (severity === "error") {
		return (
			<AlertCircleIcon className="h-3.5 w-3.5 text-red-500 dark:text-red-400 shrink-0" />
		);
	}
	if (severity === "warning") {
		return (
			<AlertTriangleIcon className="h-3.5 w-3.5 text-amber-500 dark:text-amber-400 shrink-0" />
		);
	}
	return (
		<InfoIcon className="h-3.5 w-3.5 text-blue-500 dark:text-blue-400 shrink-0" />
	);
}
		...
		<Sheet open onOpenChange={(open: boolean) => !open && onClose()}>
			...
			>
				<SheetHeader className="mb-6">
					...
					<SheetDescription>
						Paste or type your content below. SEO scores update live
						as you type — no save required.
					</SheetDescription>
				</SheetHeader>
						...
						<div className="space-y-1.5">
							...
							{result && (
								<p className="text-xs text-muted-foreground">
									{(result.wordCount ?? 0).toLocaleString()}{" "}
									words · {result.estimatedReadTime ?? 0} min
									read
								</p>
							)}
						</div>
