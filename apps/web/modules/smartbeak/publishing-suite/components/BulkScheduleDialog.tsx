...
export function BulkScheduleDialog({
	...
	const bulkMutation = useMutation(
		orpc.smartbeak.publishingSuite.bulkSchedule.mutationOptions({
			onSuccess: (data) => {
				toastSuccess(
					"Bulk schedule created",
					`${data.count} publishing jobs scheduled.`,
				);
			...
			<DialogContent className="max-w-2xl">
				...
				<div className="space-y-2">
					...
					{rows.map((row, i) => (
						<div
							key={`row-${i}`}
							className="grid grid-cols-[1fr_140px_180px_32px] items-center gap-2"
						>
							<Input
								placeholder="UUID or slug"
								value={row.contentId}
								onChange={(e) =>
									updateRow(i, { contentId: e.target.value })
								}
				...
				<DialogFooter>
					...
					>
						{bulkMutation.isPending
							? "Scheduling…"
							: `Schedule ${rows.length} Job${rows.length !== 1 ? "s" : ""}`}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
