...
function SeoScoreGauge({ score }: { score: number }) {
  ...
  return (
    <div className="relative flex h-16 w-16 shrink-0 items-center justify-center">
      ...
      <span className={`absolute text-sm font-bold tabular-nums ${color}`}>{score}</span>
    </div>
  );
}
      ...
      <CardContent className="flex gap-4 p-4">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          <div className="flex items-start justify-between gap-2">
            <h4 className="text-sm font-semibold leading-snug line-clamp-2">{idea.title}</h4>
          </div>

          <p className="text-xs text-muted-foreground line-clamp-1">{idea.outline}</p>
