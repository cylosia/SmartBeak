		...
		orpc.smartbeak.content.update.mutationOptions({
			onMutate: async (variables) => {
				...
				const previous = queryClient.getQueryData(
					orpc.smartbeak.content.get.key({
						input: { organizationSlug, id: contentId },
					}),
				);
					...
					(old: unknown) => {
						...
						return {
							...
							item: {
								...
								body:
									variables.body ??
									(data.item as Record<string, unknown>).body,
								status:
									variables.status ??
									(data.item as Record<string, unknown>)
										.status,
	...
	const handleGenerateIdeas = async () => {
		...
		try {
			...
			if (
				...
				result.data.structured.length > 0
			) {
				setAiIdeas(result.data.structured);
			} else if (
				result.success &&
				typeof result.data.ideas === "string" &&
				result.data.ideas.trim()
			) {
				try {
					const parsed = z
						.array(AiIdeaSchema)
						.parse(JSON.parse(result.data.ideas));
					setAiIdeas(parsed);
				} catch {
					setAiIdeas([]);
						...
						<Sheet>
							...
							<SheetContent>
								...
								<div className="mt-4 space-y-3">
									{(contentQuery.data?.revisions ?? [])
										.length === 0 ? (
										<p className="text-sm text-muted-foreground">
											No revisions yet. Save changes to
											create a revision.
										</p>
									) : (
										(
						...
						<Sheet>
							...
							<SheetContent>
								...
								<div className="mt-4 space-y-4">
									<p className="text-sm text-muted-foreground">
										Enter your domain or niche to generate
										content ideas.
									</p>
