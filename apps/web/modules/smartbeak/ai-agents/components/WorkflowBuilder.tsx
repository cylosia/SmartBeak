...
const AGENT_TYPE_COLORS: Record<string, string> = {
  research: "bg-blue-500/10 border-blue-500/30 text-blue-700 dark:text-blue-400",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Drag agents onto the canvas and connect them to build a workflow.
          </p>
        </div>
            ...
            agents
              ...
              .map((agent) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    addAgentNode(agent.id, agent.name, agent.agentType)
                  }
        ...
        <div className="flex-1 relative overflow-auto rounded-xl border border-border bg-muted/20 min-h-[400px]">
          ...
          >
            ...
            {graph.nodes.map((node) => {
              const nodeState = nodeStates.find((s) => s.nodeId === node.id);
              const isSelected = selectedNodeId === node.id;
              const agentType =
                agents.find((a) => a.id === node.agentId)?.agentType ??
                "custom";
                ...
                >
                  <div
                    className={`flex items-center gap-2 rounded-t-xl px-3 py-2 ${
                      AGENT_TYPE_COLORS[agentType] ?? AGENT_TYPE_COLORS.custom
                    }`}
                  >
            ...
            {graph.nodes.length === 0 && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-muted-foreground">
                ...
                <p className="text-sm">
                  Click an agent from the palette to add it to the canvas.
                </p>
              </div>
            )}
          </div>
