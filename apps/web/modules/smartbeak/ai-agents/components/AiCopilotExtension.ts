  ...
  addCommands() {
    return {
      triggerCopilot:
        ...
        ({ editor, state }) => {
          if (!this.options.enabled || !this.options.onAction) return false;
      ...
      acceptSuggestion:
        ...
        ({ state, dispatch }) => {
          ...
          if (dispatch) {
            ...
            // Clear suggestion
            tr.setMeta(COPILOT_PLUGIN_KEY, { suggestion: null, from: null });
            dispatch(tr);
          }
          return true;
        },
  ...
  addProseMirrorPlugins() {
    return [
      new Plugin({
        ...
        state: {
          init() {
            return { suggestion: null as string | null, from: null as number | null };
          },
          apply(tr, prev) {
        ...
        props: {
          decorations(state) {
            ...
            if (!pluginState?.suggestion || pluginState.from == null) {
...
export function setCopilotSuggestion(
  editor: { commands: { acceptSuggestion: () => boolean; dismissSuggestion: () => boolean } } & {
