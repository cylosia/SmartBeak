...
> = {
	research: {
		...
		description:
			"Searches the web and synthesizes information into research briefs.",
	...
	writer: {
		...
		description:
			"Transforms research into compelling, SEO-optimized content.",
	...
	editor: {
		...
		description:
			"Reviews and improves content for clarity, accuracy, and flow.",
	...
	const seedMutation = useMutation({
		...
		onSuccess: (data) => {
			if ((data as { seeded: boolean }).seeded) {
				toastSuccess(
					"Default agents created: Research, Writer, and Editor.",
				);
			} else {
				toastSuccess(
					"Agents already exist",
					"Agents already exist for this organization.",
				);
			}
				...
				<div className="flex gap-2">
					{agents.length === 0 && (
						<Button
							...
							size="sm"
							onClick={() =>
								seedMutation.mutate({ organizationSlug })
							}
			...
			{agentsQuery.isLoading ? (
				...
				<div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-20 text-muted-foreground gap-4">
					...
					<div className="text-center">
						...
						<p className="text-sm mt-1">
							Seed the three default agents or create a custom
							one.
						</p>
					</div>
					<Button
				...
				<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{agents.map((agent) => {
						const meta =
							AGENT_TYPE_META[agent.agentType] ??
							AGENT_TYPE_META.custom;
						return (
							<Card
								key={agent.id}
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div className="space-y-1.5">
								...
								>
									...
									<SelectContent>
										{Object.entries(AGENT_TYPE_META).map(
											([value, meta]) => (
												<SelectItem
													key={value}
													value={value}
