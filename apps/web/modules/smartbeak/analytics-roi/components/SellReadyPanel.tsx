...
export function SellReadyPanel({
  ...
  const readinessLabel =
    data.score >= 80 ? "Sell-Ready"
    : data.score >= 60 ? "Nearly Ready"
    : data.score >= 40 ? "Needs Work"
    : "Not Ready";

  const readinessColor =
    data.score >= 80 ? "bg-green-500/15 text-green-600 dark:text-green-400 border-green-500/20"
    : data.score >= 60 ? "bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/20"
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-base">Sell-Ready Score</CardTitle>
              ...
              <div className="flex-1 space-y-3">
                {Object.entries(data.breakdown).map(([key, value]) => {
                  const labels: Record<string, { label: string; max: number }> = {
                    ...
                    monetization: { label: "Monetization Stability", max: 20 },
                    buyerInterest: { label: "Buyer Interest", max: 30 },
                    timelineActivity: { label: "Timeline Activity", max: 20 },
                  };
                  ...
                  return (
                    <div key={key}>
                      <div className="mb-1 flex items-center justify-between text-xs">
                        ...
                        <span className="font-medium">{value} / {cfg.max}</span>
                      </div>
                      <Progress value={(value / cfg.max) * 100} className="h-1.5" />
                    </div>
                  );
                })}
              </div>
        ...
        {data.recommendations.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Address these items to increase your sell-ready score
              </CardDescription>
            </CardHeader>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-0.5 text-sm text-foreground/80">{rec.message}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <Card className="border-green-500/20 bg-green-500/5">
            <CardContent className="flex items-center gap-3 py-4">
              ...
              <p className="text-sm font-medium text-green-700 dark:text-green-300">
                This domain meets all sell-ready criteria. It is ready to list.
              </p>
            </CardContent>
          </Card>
        )}
