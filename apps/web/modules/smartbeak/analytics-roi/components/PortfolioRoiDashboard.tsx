...
function RiskBadge({ score }: { score: number }) {
  if (score >= 75) return <Badge className="bg-green-500/15 text-green-600 dark:text-green-400 border-green-500/20">Low Risk</Badge>;
  if (score >= 50) return <Badge className="bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/20">Medium Risk</Badge>;
  return <Badge className="bg-red-500/15 text-red-600 dark:text-red-400 border-red-500/20">High Risk</Badge>;
}

export function PortfolioRoiDashboard({ organizationSlug }: { organizationSlug: string }) {
    ...
    <ErrorBoundary>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-semibold">Risk-Adjusted Scores</CardTitle>
              <CardDescription>Top 10 domains by risk-adjusted ROI score</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-semibold">Portfolio Decay Trend</CardTitle>
              ...
              {trendData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={trendData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                    ...
                    <YAxis domain={[0, 1]} tick={{ fontSize: 10 }} className="fill-muted-foreground" />
                    <Tooltip
                      contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8 }}
                    />
                    <Area
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-semibold">Domain Portfolio Breakdown</CardTitle>
            <CardDescription>All domains with risk-adjusted scores and estimated values</CardDescription>
          </CardHeader>
                  ...
                  <TableRow key={domain.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Progress value={domain.healthScore ?? 0} className="h-1.5 w-20" />
                        <span className="text-xs text-muted-foreground">{domain.healthScore ?? 0}</span>
                      </div>
                    </TableCell>
                    <TableCell>
