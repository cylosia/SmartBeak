...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (domainRows.length === 0) {
      logger.warn('Domain not found or unauthorized', { domainId, orgId: auth["orgId"] });
    ...
    logger.info('Content created', { contentId, userId: auth.userId, orgId: auth["orgId"], domainId });
