 ...
 * JSON.stringify adds surrounding double-quotes; slice(1,-1) removes them,
 * leaving only the correctly-escaped interior value.
 *
 * P1-FIX: replaceTemplateVars was applying escapeHtml() to {{siteName}} inside
 * package.json (a JSON file). HTML escaping converts & → &amp;, " → &quot; etc.
 * In a JSON string literal, &amp; is a literal five-character sequence — the wrong
 * value — and &quot; would corrupt the JSON by introducing an unescaped backslash.
 * JSON contexts must use JSON-string escaping, not HTML escaping.
 */
 ...
 * SECURITY FIX P0 #6: All user-provided values are escaped for their context.
 *
 * P1-FIX: replaceTemplateVars is only called for BASE_TEMPLATE files (package.json,
 * next.config.js, tsconfig.json) — all of which are JSON or JS, not HTML.
 * Applying escapeHtml() to JSON string values is wrong: it converts & to &amp;,
 * " to &quot; etc., producing HTML entities as literal content in the JSON value.
 * In particular &quot; inside a JSON string literal would corrupt the JSON structure.
 * Use escapeForJsonString() which applies proper JSON-string escaping instead.
 * HTML-context pages are generated by the separate generateXxxPage() functions
 * which correctly call escapeHtml() directly.
 */
function replaceTemplateVars(content: string, config: ThemeConfig): string {
  return content
    .replace(/\{\{siteName\}\}/g, escapeForJsonString(config.siteName))
    .replace(/\{\{siteDescription\}\}/g, escapeForJsonString(config.siteDescription || config.siteName))
    .replace(/\{\{primaryColor\}\}/g, validateCssColor(config.primaryColor))
    .replace(/\{\{logoUrl\}\}/g, escapeForJsonString(config.logoUrl || ''));
}
