...
-- Audit events table (high volume time-series data) - skip if table absent
DO $$ BEGIN
...
-- Analytics events table (very high volume) - skip if table absent
DO $$ BEGIN
...
-- Usage metrics table (high volume time-series) - skip if table absent
DO $$ BEGIN
...
-- Cost optimization events - skip if table absent
DO $$ BEGIN
...
-- Comment explaining BRIN index benefits (skip if indexes don't exist)
DO $$ BEGIN
