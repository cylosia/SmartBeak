...
export class NotificationSent {
  ...
  // treat 'notification.sent' as an unreachable branch, hiding unhandled event bugs.
  ...
  toEnvelope(notificationId: string, correlationId?: string): DomainEventEnvelope<'notification.sent', { notificationId: string }> {
  return {
    id: randomUUID(),
