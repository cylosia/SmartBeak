...
import {
  ArrowLeft, Rocket, ExternalLink, Clock, Layers,
  LayoutTemplate, Eye, CheckCircle2, XCircle,
  Loader2, Globe, FileText, RefreshCw,
} from "lucide-react";
import type { Domain, SiteShard, DeploymentVersion, AuditLog, ThemeOption } from "@shared/schema";
    ...
    case "pending":
      return (
        <Badge variant="secondary" className="bg-amber-500/10 text-amber-700 dark:text-amber-400">
          ...
          {status === "pending" ? "Queued" : status === "building" ? "Building" : "Deploying"}
        </Badge>
      );
    case "error":
      return (
...
export default function DomainDetailPage() {
  ...
  const isDeploying = domain?.latestShard &&
    ["pending", "building", "deploying"].includes(domain.latestShard.status);
  ...
  const deployMutation = useMutation({
    mutationFn: async ({ theme }: { theme: string }) => {
      const res = await apiRequest("POST", `/api/domains/${id}/deploy`, { theme });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/domains", id] });
      toast({ title: "Deployment started", description: "Your site is being deployed to Vercel." });
      setDeployOpen(false);
    },
    onError: (err: Error) => {
  ...
  if (!domain) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-16 text-center">
        ...
        <p className="text-muted-foreground mb-4">The domain you're looking for doesn't exist.</p>
  ...
  const domainAuditLogs = auditLogs?.filter(
    (log) => log.entityId === domain.id || domain.shards.some((s) => s.id === log.entityId)
  ) || [];
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center gap-3 mb-6">
          <Button variant="ghost" size="icon" data-testid="button-back" onClick={() => setLocation("/")}>
            <ArrowLeft className="w-4 h-4" />
          </Button>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold tracking-tight truncate" data-testid="text-domain-name">
                {domain.name}
              </h1>
              {latestShard && <StatusBadge status={latestShard.status} />}
            </div>
            {domain.description && (
          ...
          <div className="flex items-center gap-2">
            {latestShard?.deployedUrl && (
              <Button
                ...
                onClick={() => window.open(latestShard.deployedUrl!, "_blank")}
              >
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between gap-4 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">Deployment in Progress</span>
                </div>
                <span className="text-sm text-muted-foreground" data-testid="text-deploy-progress">
                  {latestShard.progress}%
                </span>
              </div>
              <Progress value={latestShard.progress} className="h-2 mb-2" data-testid="progress-deploy" />
              <p className="text-xs text-muted-foreground">
                {latestShard.status === "pending" && "Preparing deployment..."}
                {latestShard.status === "building" && "Generating static site from template..."}
                {latestShard.status === "deploying" && "Deploying to Vercel edge network..."}
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <div className="grid gap-4 md:grid-cols-3 mb-6">
          <Card>
            <CardContent className="p-5">
              ...
              <p className="font-semibold" data-testid="text-domain-theme">{THEME_LABELS[domain.theme] || domain.theme}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              ...
              <p className="font-semibold" data-testid="text-deployment-count">{domain.shards.length} total</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                <Clock className="w-4 h-4" />Latest Version
              </div>
              <p className="font-semibold" data-testid="text-latest-version">
                {latestShard ? `v${latestShard.version}` : "No deployments yet"}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="preview" className="space-y-4">
          ...
          <TabsContent value="preview">
            {latestShard?.deployedUrl && latestShard.status === "ready" ? (
              <Card>
                ...
                <CardContent className="p-0">
                  <div className="relative rounded-b-md border-t" style={{ height: "500px" }}>
                    <iframe
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2" data-testid="text-no-preview">No preview available</h3>
            ...
            <Card className="mt-4">
              ...
              <CardContent className="p-0">
                <div className="relative rounded-b-md border-t" style={{ height: "400px" }}>
                  <iframe
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center shrink-0">
                            <span className="text-xs font-bold">v{shard.version}</span>
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-medium text-sm">
                                {THEME_LABELS[shard.theme] || shard.theme}
                              </span>
                              <StatusBadge status={shard.status} />
                            </div>
                            <div className="text-xs text-muted-foreground mt-0.5">
                              {new Date(shard.createdAt).toLocaleString()}
                              {shard.deployedUrl && (
                                <span> · <a
                                ...
                                >{shard.deployedUrl}</a></span>
                              )}
                            </div>
                            {shard.errorMessage && (
                              <p className="text-xs text-destructive mt-1">{shard.errorMessage}</p>
                            )}
                          </div>
                        </div>
                        {shard.deployedUrl && shard.status === "ready" && (
                          <Button
                            variant="outline"
                            size="sm"
                            data-testid={`button-open-shard-${shard.id}`}
                            onClick={() => window.open(shard.deployedUrl!, "_blank")}
                          >
                  ...
                  <ScrollArea className="h-80">
                    <div className="space-y-3">
                      {domainAuditLogs.map((log) => (
                        <div key={log.id} className="flex items-start gap-3" data-testid={`audit-log-${log.id}`}>
      ...
      <Dialog open={deployOpen} onOpenChange={setDeployOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose a theme and deploy a production-ready static site to Vercel.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="space-y-2">
              <Label>Theme Template</Label>
              <Select value={deployTheme} onValueChange={setDeployTheme}>
            ...
            <div className="rounded-md border bg-muted/30 p-3">
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                ...
                {latestShard && <p>Next version: v{latestShard.version + 1}</p>}
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => deployMutation.mutate({ theme: deployTheme })}
              disabled={deployMutation.isPending}
            >
              <Rocket className="w-4 h-4 mr-2" />
              {deployMutation.isPending ? "Starting..." : "Deploy to Vercel"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-5xl h-[85vh]">
          <DialogHeader>
            ...
            <DialogDescription>Full preview of your deployed site</DialogDescription>
          </DialogHeader>
          {latestShard?.deployedUrl && (
            <iframe
              src={latestShard.deployedUrl}
