...
import type { ThemeOption, SiteShard } from "@shared/schema";
...
export async function deployToVercel(
  ...
  (async () => {
    try {
      await storage.updateSiteShard(shard.id, { status: "building", progress: 10 });

      const html = generateThemeHtml(theme, domainName);
      await storage.updateSiteShard(shard.id, { progress: 30 });

      log(`Generated HTML for ${domainName} with theme ${theme}`, "deploy");

      const projectName = sanitizeProjectName(`smartbeak-${domainName}-${Date.now()}`);
      ...
      await storage.updateSiteShard(shard.id, { status: "deploying", progress: 50 });
      ...
      log(`Vercel deployment created: ${deployData.id} - ${deployData.url}`, "deploy");
      ...
      while (!ready && attempts < maxAttempts) {
        ...
        const statusRes = await vercelFetch(`/v13/deployments/${deployData.id}`);
        if (!statusRes.ok) {
          continue;
        }
        const statusData = await statusRes.json() as { readyState: string };
        const currentProgress = Math.min(70 + Math.floor((attempts / maxAttempts) * 25), 95);
        await storage.updateSiteShard(shard.id, { progress: currentProgress });

        if (statusData.readyState === "READY") {
          ready = true;
        } else if (statusData.readyState === "ERROR" || statusData.readyState === "CANCELED") {
          throw new Error(`Deployment ${statusData.readyState.toLowerCase()}`);
        }
      }
