...
export const siteShardsRelations = relations(siteShards, ({ one, many }) => ({
  domain: one(domains, { fields: [siteShards.domainId], references: [domains.id] }),
  versions: many(deploymentVersions),
}));

export const deploymentVersions = pgTable("deployment_versions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  shardId: varchar("shard_id").notNull().references(() => siteShards.id, { onDelete: "cascade" }),
...
export const deploymentVersionsRelations = relations(deploymentVersions, ({ one }) => ({
  shard: one(siteShards, { fields: [deploymentVersions.shardId], references: [siteShards.id] }),
}));
